...
ARoad::ARoad()
{
	...
	PathWayWest->AttachTo(RootComponent);
	...
	PathWayEast->AttachTo(RootComponent);
	...
	PathWayNorth->AttachTo(RootComponent);
	...
	PathWaySouth->AttachTo(RootComponent);
...
void ARoad::UpdateSideOfRoadTile()
{
	...
	bool bHit = GetWorld()->LineTraceSingleByChannel(Hit, RoadStartRayLoc, RoadEndRayNorthLoc, ECC_Visibility, TraceParams);
